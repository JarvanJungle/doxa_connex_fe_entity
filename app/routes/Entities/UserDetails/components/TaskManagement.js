...
export default function TaskManagement(props) {
    ...
    useEffect(() => {
        if (listModules && listModules.length > 0 && gridApi) {
            const newListFeature = listModules.map((item) => ({
                ...item,
                disabled
            }));
            gridApi.setRowData(newListFeature);
        }
    }, [disabled, listModules, gridApi]);
                        ...
                        <Row className="mb-3">
                            <Col lg={12}>
                                <AgGridTable
                                    ...
                                    rowData={listModules}
                                    gridHeight={listModules?.length > 1 ? 500 : 190}
                                    sizeColumnsToFit
                                    frameworkComponents={{
                                    ...
                                    gridOptions={{
                                        ...
                                        getRowNodeId: (data) => data?.featureCode
                                    }}
