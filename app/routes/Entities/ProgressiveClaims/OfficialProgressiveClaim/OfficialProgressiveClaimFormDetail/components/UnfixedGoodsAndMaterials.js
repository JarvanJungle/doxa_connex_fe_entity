...
const editableFields = {
    ...
    cetifiedRemarks: {
        canEdit: false,
        showOnStatus: [PC_STATUS.CREATED, PC_STATUS.PENDING_ISSUE]
    },
...
const UnfixedGoodsAndMaterials = (props) => {
    ...
    const getFirstGroupNumber = (item) => (Array.isArray(item.groupNumber)
        ? item.groupNumber.at(0)
        : item.groupNumber);
    ...
    const calculateSummary = () => {
        const totalAmount = {};
        getAllRowNodeDatas().map((item) => {
            if (item.groupNumber.length === 1) {
                totalAmount[getFirstGroupNumber(item)] = 0;
            }
            totalAmount[getFirstGroupNumber(item)] += (Number(item.mainConClaimQty) * Number(item.mainConClaimUnitPrice));
            return item;
        });

        getAllRowNodes().map((itemMap) => {
            if (
                totalAmount[getGroupNumber(itemMap.data)]
                && itemMap.data.mainConClaimTotalAmount
                !== totalAmount[getGroupNumber(itemMap.data)]
            ) {
                itemMap.setDataValue(
    ...
    const GroupCellRenderer = (params) => {
        ...
        if (data?.groupNumber) {
            const isShowAction = ![PC_STATUS.PENDING_ACKNOWLEDGEMENT].includes(draftDetail?.pcStatus);
            const {
                groupNumber,
                quantity,
                unitPrice,
    ...
    const downLoadFile = async (data) => {
        try {
            const response = await EntityServices.downloadDocuments("purchase-service/documents", data.guid || data.attachment);
    ...
    const AddAttachment = (params) => {
        ...
        return (
            <>
                {(!data.attachment && [PC_STATUS.CREATED, PC_STATUS.PENDING_ISSUE].includes(data?.pcStatus)) ? (
                    <>
                        <input
                            ref={fileInput}
                    ...
                    <Row className="mb-3">
                        <Col xs={12}>
                            <ButtonToolbar className="justify-content-end">
                                ...
                                {!onlyShowDownload
                                    && (
                                        values.pcStatus === PC_STATUS.CREATED
                                        || values.pcStatus === PC_STATUS.PENDING_ISSUE
                                    ) && (
                                    <>
                                        <Button color="primary" className="mr-1">
                                            <i className="fa fa-download mr-2" />
