...
const UnfixedGoodsAndMaterials = (props) => {
    ...
    const GroupCellRenderer = (params) => {
        ...
        if (data?.groupNumber) {
            const isShowAction = ![PC_STATUS.PENDING_ACKNOWLEDGEMENT].includes(draftDetail?.aceStatus);
            const {
                groupNumber,
                quantity,
                unitPrice,
    ...
    const AddAttachment = (params) => {
        ...
        return (
            <>
                {(!data.attachment && [PC_STATUS.CREATED, PC_STATUS.PENDING_ISSUE].includes(draftDetail?.aceStatus)) ? (
                    <>
                        <input
                            ref={fileInput}
                    ...
                    <Row className="mb-3">
                        <Col xs={12}>
                            <ButtonToolbar className="justify-content-end">
                                ...
                                {!onlyShowDownload
                                    && (
                                        values.aceStatus === PC_STATUS.CREATED
                                        || values.aceStatus === PC_STATUS.PENDING_ISSUE
                                    ) && (
                                        <>
                                            <Button color="primary" className="mr-1">
                                                <i className="fa fa-download mr-2" />
